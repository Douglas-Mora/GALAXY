...
# This is the apps interface launcher.
...
class MainWidget(RelativeLayout):
    ...
    def reset_game(self):
        ...
        with open("score.csv", "r") as file_to_read:
            reader = csv.reader(file_to_read)
            for i in reader:
                if len(i) >= 1:
                    number = int(i[0])
        if self.current_y_loop > number:
            with open("score.csv", "w") as file_to_write:
                writer = csv.writer(file_to_write)
                writer.writerow([self.current_y_loop])
        ...
        self.score_txt = "SCORE: " + str(0)
        self.max_score_txt = "MAX: " + str(number)
    ...
    def update(self, dt):
        ...
        if not self.state_game_over and self.state_game_has_started:
            self.SPEED += self.current_y_loop / 1500000
